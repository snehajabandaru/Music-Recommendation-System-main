...
def crop(clahe_image, face):
    for (x, y, w, h) in face:
        ...
        faceslice=cv.resize(faceslice, (350, 350))
...
def grab_face():
    ...
    cv.imwrite("images/main%s.jpg" %count, frame)
...
def detect_face():
    ...
    face=facecascade.detectMultiScale(clahe_image, scaleFactor=1.1, minNeighbors=15, minSize=(10, 10), flags=cv.CASCADE_SCALE_IMAGE)
    if len(face)>=1:
...
def save_face(emotion):
    ...
    for i in facedict.keys():
        ...
        cv.imwrite("dataset/%s/%s.jpg" %(emotion, (file_count)), facedict[i])
    facedict.clear()
...
def identify_emotions():
    ...
    for i in facedict.keys():
        ...
        cv.imwrite("images/%s.jpg" %i, facedict[i])
